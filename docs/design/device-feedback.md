...
- **一致時は無通知**（OK通知は出さない。端末は必要に応じてプルで明細確認）。不一致時のみプッシュする。
...
```json
{
  ...
  "type": "inventory",
  "locations": [
    { "location": { "l1": "TOKYO-DC", "l2": "2F", "l3": "A-01" }, "total_quantity": 1200,
      "items": [ { "sku": "ITEM-AAA", "quantity": 800 }, { "sku": "ITEM-BBB", "quantity": 400 } ] },
    { "location": { "l1": "TOKYO-DC", "l2": "2F", "l3": "A-02" }, "total_quantity": 530, "items": [ ] }
  ],
  "as_of": "2026-06-18T12:34:56Z"
}
```
...
### 決定済み
1. **到達性一致時の通知**: 出さない（不一致時のみプッシュ。§2）。
2. **一次手段**: 完了イベントは **MQTT** で送る前提とし、到達性フィードバックは **C2Dプッシュ**を一次手段とする（HTTP同期応答方式は採らない）。
